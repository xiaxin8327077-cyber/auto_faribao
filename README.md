...
LongCat 仅用于现有指令的自然语言路由、普通问题聊天和只读线上诊断，不参与日报提交逻辑、净值计算、定时调度或消息去重。
...
现有确定性指令始终优先，不调用模型；“今天我的理财能涨吗”等涨跌询问会优先进入现有持仓预估功能。无法匹配系统指令的普通问题自动进入聊天，`问助手` 和助手模式仍保留兼容。系统有指令正在执行时，聊天、自然语言路由和线上诊断都不会调用 LongCat。模型识别出的设置、删除、提交等写操作需要回复 `确认执行`，也可以回复 `取消执行`。
