...
### 26. 企业微信 AI 智能助手（默认关闭）

AI 模型仅用于现有指令的自然语言路由、普通问题聊天和只读线上诊断，不参与日报提交逻辑、净值计算、定时调度或消息去重。支持百炼 `qwen3.7-plus` 与 LongCat 热切换，LongCat 可作为备用模型保留。
...
AI_PROVIDER=qwen
AI_DIAGNOSTIC_USERS=*
QWEN_API_KEY=replace-with-server-secret
...
- `AI_PROVIDER` 支持 `qwen` 或 `longcat`；企微发送 `切换AI模型 百炼`、`切换AI模型 LongCat` 可在连通检测成功后立即切换，选择写入 `data/ai_provider_state.json` 并在服务重启后保留。
- 未设置 `AI_ENABLED=true`（兼容旧的 `LONGCAT_ENABLED=true`）或两个提供商均未配置密钥时，AI功能关闭，系统保持原有行为。
- `AI_DIAGNOSTIC_USERS` 填写允许读取脱敏日志和源码的企业微信用户ID，多个用户使用英文逗号分隔；配置为 `*` 时不限制企业微信用户，未配置时任何用户都不能发起线上诊断。
...
现有确定性指令始终优先，不调用模型；“今天我的理财能涨吗”等涨跌询问会优先进入现有持仓预估功能。无法匹配系统指令的普通问题自动进入聊天，`问助手` 和助手模式仍保留兼容。系统有指令正在执行时，聊天、自然语言路由和线上诊断都不会调用 AI 模型。模型识别出的设置、删除、提交等写操作需要回复 `确认执行`，也可以回复 `取消执行`。

线上诊断采用两步模型流程：首次调用规划最多 3 个只读工具，系统批量执行并脱敏，第二次调用直接生成诊断报告。诊断限制为单并发和 30 秒总时限。源码读取限制在项目 Python 文件内，禁止访问 `config.yaml`、`.env`、`data/`、`runtime/`、`backups/`、虚拟环境、Cookie和密钥；模型不能修改文件、配置、数据库或服务。
