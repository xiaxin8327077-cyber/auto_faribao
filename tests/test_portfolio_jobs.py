...
def test_cycle_runs_network_sync_before_database_settlement():
    ...
    jobs = PortfolioJobs(
        ...
        settle_redemptions=lambda _day: calls.append("redemptions"),
        accrue_income=lambda _day: calls.append("income"),
    )
    jobs.run_cycle(datetime(2026, 7, 30, 18, 0))
    assert calls == [
        "quotes", "intents", "settle", "redemptions", "settle", "income",
    ]
...
def test_cycle_confirms_then_settles_redemptions_then_confirms_again():
    ...
    jobs = PortfolioJobs(
        ...
        settle_redemptions=lambda _day: calls.append(
...
def test_cycle_is_idempotent_within_same_thirty_minute_slot():
    ...
    assert first == PortfolioCycleResult(0, 0, 0, 0, 0)
    assert second == PortfolioCycleResult(0, 0, 0, 0, 0)
    assert calls == [
        "quotes", "intents", "settle", "redemptions", "settle", "income",
    ]
...
def test_cycle_runs_again_in_next_thirty_minute_slot():
    ...
    assert calls.count("settle") == 4
    assert calls.count("redemptions") == 2
...
def test_cycle_records_returned_counts():
    jobs = PortfolioJobs(
        ...
        settle_redemptions=lambda _day: 5,
        accrue_income=lambda _day: 4,
    )
    result = jobs.run_cycle(datetime(2026, 7, 30, 9, 0))
    assert result == PortfolioCycleResult(3, 1, 4, 5, 4)
