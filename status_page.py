...
function fmtDateTime(value){ const text=String(value ?? '').trim(); if(!text){ return '-'; } const normalized=text.replace('T',' ').replace(/(?:\.\d+)?(?:Z|[+-]\d{2}:?\d{2})$/,''); return normalized.length >= 19 ? normalized.slice(0,19) : normalized; }
...
async function loadPasswordInfo(){ if(ROLE !== 'full'){ return; } const r=await fetch('/api/password-info', {cache:'no-store'}); if(r.status===401){ location.href='/'; return; } const data=await r.json(); setText('pwdUpdatedAt', fmtDateTime(data.updated_at)); setText('pwdStorage', data.storage === 'password_hash' ? '密码哈希' : data.storage); }
